...
function renderLastOrders(list){
  ...
  if (q){
    rows = rows.filter(o => {
      ...
      const nome =
        String(o.data?.nome || o.data?.customer?.nome || "").toLowerCase();
      const email =
        String(o.data?.email || o.data?.customer?.email || "").toLowerCase();
  ...
  lastOrdersEl.innerHTML = rows.map(o => {
    const data = o.data || {};
    const nome = data.nome || data.customer?.nome || "Cliente";
...
async function carregarDashboard(){
  ...
  snap.forEach(doc => {
    const data = doc.data();
    const total = Number(data.total ?? data.pricing?.total ?? 0);
    totalVendas += total;
  ...
  ticketMedioEl.textContent = brl(all.length ? totalVendas / all.length : 0);

  renderStatusBreakdown(all);

  try{
    const q = query(collection(db, "orders"), orderBy("createdAt","desc"), limit(12));
    ...
    console.warn("[DASH] fallback local:", err);
